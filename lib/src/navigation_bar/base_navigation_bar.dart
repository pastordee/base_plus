...
class BaseNavigationBarAction {
  ...
  IconData? _sfSymbolToMaterialIcon(String? name) {
    ...
    // Nav-bar-specific looks (e.g. iOS-style back-arrow chevrons) take
    // precedence; everything else delegates to the shared base_plus map so new
    // symbols are covered everywhere at once. Returns null when unknown so the
    // caller's own default icon still applies.
    const Map<String, IconData> overrides = {
    ...
    return overrides[name] ?? kSfSymbolMaterialIcons[name];
  }
}
...
class BaseNavigationBar extends BaseStatelessWidget {
  ...
  IconData _mapSFSymbolToMaterialIcon(String sfSymbol) {
    // Delegates to the shared base_plus SF Symbol → Material map so coverage is
    // consistent across every component (see sf_symbol_material_icons.dart).
    return sfSymbolToMaterialIcon(sfSymbol, fallback: Icons.circle);
  }
}
